...
async function loadSettings(){const s=await fetch("/api/settings").then(r=>r.json());currentSettings=s;for(const [k,v]of Object.entries(s)){if(k==="ledMode"){const selected=form.querySelector(`[name=ledMode][value="${v}"]`);if(selected)selected.checked=true;continue}if(form.elements[k])form.elements[k].value=k.includes("Threshold")?pct(v):v;if(form.elements[k+"Number"])form.elements[k+"Number"].value=pct(v)}$("#modeStatus").textContent=modeNames[s.ledMode]||"--"}
